...
class GlucoseHistory:
    ...
    def get_obs(self):
        return np.array([self.obs_queue[1], self.obs_queue[1] - self.obs_queue[0], self.time])
...
class CSTR(gym.Env):
    ...
    def __init__(
        ...
        model.set_rhs(
            "T_R",
            ((K_1 * C_a * H_R_ab + K_2 * C_b * H_R_bc + K_3 * (C_a**2) * H_R_ad) / (-Rou * Cp))
            + F_r * (T_in - T_R)
...
class CartPole(CartPoleEnv):
    def __init__(self, altered_paras={}, render_mode=None, max_steps=250, xp=1):
        ...
        self.init_state = np.array([0.0, 0.0, 6 * 2 * math.pi / 360, 0], dtype=np.float32)
        self.cost_space = gym.spaces.Box(
    ...
    def step(self, action):
        ...
        temp = (
            action.item() * self.force_mag + self.polemass_length * theta_dot**2 * sintheta
        ) / self.masstotal
        ...
        terminated = bool(
            x < -4.8 or x > 4.8 or theta < -24 * math.pi / 360 or theta > 24 * math.pi / 360
        )
        if terminated:
            reward -= 1e4
