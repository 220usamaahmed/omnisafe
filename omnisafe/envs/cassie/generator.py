...
class FootTrajectoryGenerator:
    ...
    def getJointTargets(self, t, freq_list=np.zeros(2)):
        self.phi_list[:] = np.mod(
            self.init_phi_list + (freq_list + self.default_freq) * t, 2.0 * np.pi
        )
        ...
        for leg_idx in range(self.num_leg):
            ...
            if k < 0:
                ...
                pos = (
                    self.foot_height * (2.0 * (k**3) - 9.0 * (k**2) + 12.0 * k - 4.0) + self.bottom
                )
            ...
            theta3 = (
                np.arcsin(
                    (target_x**2 + target_y**2 - (self.a**2 + self.b**2 + self.l3**2))
                    / (2 * self.l3 * np.sqrt(self.a**2 + self.b**2))
                )
                - self.default_theta
                - self.d3
            )
            theta2 = np.arctan2(target_y, target_x) - np.arctan2(
                self.l3 * np.sin(theta3 + self.d3) + self.l4 * np.sin(self.d4),
                self.l2 + self.l3 * np.cos(theta3 + self.d3) + self.l4 * np.cos(self.d4),
            )
